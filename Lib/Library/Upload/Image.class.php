...
class Image {
     ...
     * @param string $mod
     */
    static function thumb($image, $thumbname, $maxWidth = 200, $maxHeight = 150, $mod='auto', $type='') {
        ...
        if ($info !== false) {
            ...
            switch ($mod) {
                ...
                case 'both':
                    ...
                    if(($srcWidth / $srcHeight) > ($maxWidth / $maxHeight)){
                        ...
                        $width = ($maxHeight * $srcWidth) / $srcHeight;
                        $height = $maxHeight;
                    ...
                    // 高度大于比例，取小值的宽度，后需剪切
                        ...
                        $width = $maxWidth;
                        $height = ($maxWidth * $srcHeight) / $srcWidth;
                    }
            ...
            // 判断是否存在该方法并执行
            if(!function_exists($createFun)) return false;
            ...
            // 如果模式处于both，这里加入图片剪切
            if ($mod == 'both') {
                $squareImg = imagecreatetruecolor($maxWidth, $maxHeight);
                imagecopy($squareImg, $thumbImg, 0, 0, ($width - $maxWidth) / 2, ($height - $maxHeight) / 2, $maxWidth, $maxHeight);
            ...
            if ('gif' == $type || 'png' == $type) {
                ...
                $background_color = imagecolorallocate($thumbImg, 0, 0, 0);  //  指派一个黑色
     ...
     * @return void
     */
    static function thumb_old($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true) {
        // 获取原图信息
        $info = Image::getImageInfo($image);
        if ($info !== false) {
            $srcWidth = $info['width'];
